    ...
    if(not(per)):
        ...
        def ok():
            try:
                ...
                dff = pd.DataFrame(dt)
                new_win_label = Label(new_win, text=f"{dff}", font=("Arial", 14, "bold"))
                new_win_label.grid(row=0,column=1)
                # p = 2
                unique_button = Button(new_win,text="Unique values",font=("Arial",13,"bold"),width=15)
                unique_button.grid(row=len(data[column_input1]),column=1,pady=10,padx=10)
